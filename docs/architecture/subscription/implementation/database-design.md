...
**`Promotion`**, **`PromotionRedemption`** (Phase VII, **implemented 2026-09-26**; formerly S14; migrations `20260926000000`–`20260926000200`): `Promotion { id, code @unique (stored normalized, CHECK), plan, durationDays (CHECK > 0), remainingRedemptions Int?, validFrom, validUntil? (CHECK: after validFrom), eligibility (enum CodeEligibility), createdByUserId, createdAt }`; `PromotionRedemption { id, promotionId Restrict, userId Restrict + subjectPseudonym, grantId @unique, createdAt, @@unique([promotionId, userId]) }`; CHECK `"remainingRedemptions" IS NULL OR "remainingRedemptions" >= 0`.
