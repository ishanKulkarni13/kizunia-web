...
| `PENDING_AUTHENTICATION`, other plan/cycle or expired | Composed command: cancel it (confirmed by sync), then proceed. If Razorpay refuses to cancel a `created` subscription ([A1](../../../project/feature-specification/subscription/open-decisions.md#a-razorpay-behavior-requiring-test-mode-verification-or-support)), the new checkout waits until the old one's `expire_by` has passed and been observed; the user may continue the old checkout meanwhile |
