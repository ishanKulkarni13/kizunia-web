...
| A6 | Whether `x-razorpay-event-id` is present on **every** delivery and **identical across retries** | TEST | **[DOC]** unique per event, recommended for de-duplication, at-least-once delivery; retries of pre-rotation events use the old secret. **Not documented:** presence on every delivery; equality across retries (the best-practices page is silent). **[INFERENCE]** equality follows from "unique per event". **[TEST, 2026-09-25]** verified for `subscription.cancelled`: present on every delivery seen and identical across six attempts and a replay ([Phase IV run](../../../architecture/subscription/provider-boundary/razorpay-facts.md#phase-iv-webhook-run-2026-09-25)) | Nothing further for that event; other types were not exercised | A delivery without the header is deduplicated on `sha256(raw body)` instead | [SB-WH-02](decisions/webhooks-and-reliability.md#sb-wh-02--every-received-event-is-persisted-with-a-unique-constraint-dedupe-key-before-acknowledgement) dedupe key |
...
| A8 | How does `expire_by` interact with `expired` for a `created` subscription without `start_at`? | TEST | It becomes **`expired`** after `expire_by`, but only after a lag of several minutes (188 s, 322 s and about 156 s observed; size windows for ~6 min, D6). `expire_by` ≥ 30 s ahead accepted; past values rejected | Behavior of a payment made inside the lag window not tested |
...
| C5 | Checkout `expire_by` horizon and lease duration | **E** | Chosen in Phase V: `expire_by` = now + 30 min, operation lease 60 s (configuration.md). `created` can read `created` for several minutes after `expire_by` (A8/D6) |
