...
| `NOT_FOUND` | 404 (**observed:** only a malformed ID, as a gateway routing miss; a real unknown ID is a `400` that classifies as `REJECTED`, so this class does not by itself mean "the subscription is gone", see D12 in [razorpay-facts](../provider-boundary/razorpay-facts.md#documentation-vs-observed-behavior); Phase IV rules how a missing subscription is detected) | No | Unchanged; `PROVIDER_MODE_MISMATCH` or `PROVIDER_SUBSCRIPTION_MISSING` anomaly, **once detection is ruled** | Always |
