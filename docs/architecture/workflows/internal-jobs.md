...
| 5 | `billing:payload-prune` (Phase VIII) | 24 h (`BILLING_PAYLOAD_PRUNE_MIN_INTERVAL_SECONDS`) | 3 s soft (`BILLING_PAYLOAD_PRUNE_WALL_CLOCK_MS`), at most 20 batches of 500 rows, database only |
| 6 | `billing:orphan-discovery` (Phase V) | 900 s (`BILLING_ORPHAN_MIN_INTERVAL_SECONDS`) | 5 s soft (`BILLING_ORPHAN_WALL_CLOCK_MS`), at most 3 list pages; takes whatever the tasks before it leave |
...
nothing and the next run resumes. `billing:payload-prune` (Phase VIII) runs daily just before it: database
only, bounded, and small enough not to disturb the worst case above. Reaching the billing target cadence (about 5 minutes) on the
...
| `GET /api/v1/internal/billing/payload-prune` | Standard (above) | No — runs as the `billing:payload-prune` task; route kept for manual runs |
