...
**Decisions referenced here:** [IB-2](open-decisions.md#ib-2--recommendation-gate-point), [IB-7](open-decisions.md#ib-7--admin-bypass-of-entitlement-gates), [IB-12](open-decisions.md#ib-12--soft-deleted-projects-and-the-quota). All were ruled on 2026-09-24; see [open decisions](open-decisions.md) for each ruling and who made it (product decision (owner) or architecture decision (autonomous)).
...
- **Admin override (IB-7, product decision (owner)):** interactive gates (quota, portfolio create, MCP) call `.platformOverride()` (`ADMIN`/`SUPER_ADMIN`). Background notification eligibility has no bypass; admins who want those notifications get an admin grant from another admin. Customers get extra access via an admin grant, never a code path.
...
- **No plan logic in features:** features import `Capability` and the per-user effective-access API (IB-3) or `hasCapability` only. A lint rule forbids importing `MembershipPlan` outside `lib/entitlements` and `modules/billing`.
